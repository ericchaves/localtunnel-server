...
describe('TunnelAgent', () => {
    ...
    it('should reject connections over the max', async () => {
        ...
        // The server now sends HTTP 429 response with JSON body before closing
        // So we need to wait for data first, then close
        const dataReceived = new Promise(resolve => {
            sock3.once('data', (data) => {
                const response = data.toString();
                // Verify it's a 429 response
                assert(response.includes('429'), 'Should receive 429 response');
                assert(response.includes('Too Many Connections'), 'Should mention connection limit');

                // Verify JSON body
                assert(response.includes('application/json'), 'Should have JSON content type');

                // Extract and parse JSON body
                const bodyMatch = response.match(/\r\n\r\n(.+)$/);
                if (bodyMatch) {
                    const jsonBody = JSON.parse(bodyMatch[1]);
                    assert.equal(jsonBody.error, 'Too Many Connections', 'Should have error field');
                    assert.equal(jsonBody.code, 429, 'Should have code 429');
                    assert.equal(jsonBody.max_sockets, 2, 'Should report max_sockets=2');
                    assert.equal(jsonBody.current_sockets, 2, 'Should report current_sockets=2');
                }
        ...
        const closeReceived = new Promise(resolve => sock3.once('close', resolve));
    ...
    // New tests for specific port functionality
    it('should listen on a specific port when provided', async () => {
        const specificPort = 9000;
        const agent = new TunnelAgent({
            port: specificPort,
        });

        const info = await agent.listen();
        assert.equal(info.port, specificPort);
        agent.destroy();
    });

    it('should listen on random port when no port is provided', async () => {
        const agent = new TunnelAgent();
        const info = await agent.listen();

        // Random port should be greater than 0
        assert.ok(info.port > 0);
        agent.destroy();
    });

    it('should accept connections on the specific port', async () => {
        const specificPort = 9001;
        const agent = new TunnelAgent({
            port: specificPort,
        });
        ...
        const sock = net.createConnection({ port: specificPort });
        await new Promise(resolve => sock.once('connect', resolve));
        ...
        agentSock.write('test data');
        await new Promise(resolve => sock.once('readable', resolve));

        assert.equal('test data', sock.read().toString());
        agent.destroy();
        sock.destroy();
    });

    it('should store the port in the agent instance', async () => {
        const specificPort = 9002;
        const agent = new TunnelAgent({
            port: specificPort,
        });

        assert.equal(agent.port, specificPort);
