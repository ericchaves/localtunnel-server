...
class Client extends EventEmitter {
    ...
    handleRequest(req, res) {
        ...
        const timeout = setTimeout(() => {
            this.debug('Request timeout after %dms - destroying request', REQUEST_TIMEOUT);
        ...
        clientReq.once('error', (err) => {
            ...
            if (res.headersSent) {
                this.debug('Request error after headers sent: %s', err.message);
                return;
            }

            // Respond with 503 Service Unavailable
            this.debug('Request error: %s - responding 503', err.message);
            res.statusCode = 503;
    ...
    handleUpgrade(req, socket) {
        ...
        const timeout = setTimeout(() => {
            this.debug('WebSocket upgrade timeout after %dms', WEBSOCKET_TIMEOUT);
            socket.end();
        }, WEBSOCKET_TIMEOUT);
        ...
        this.agent.createConnection({}, (err, conn) => {
            ...
            if (err) {
                this.debug('WebSocket upgrade error: %s', err.message);
                socket.end();
                return;
            }
            ...
            if (!socket.readable || !socket.writable) {
                this.debug('WebSocket socket closed while waiting');
                conn.destroy();
                socket.end();
                return;
            }
