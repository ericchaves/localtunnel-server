...
class ClientManager {
    ...
    async newClient(id, options = {}) {
        ...
        this.debug('newClient called with: requestedId=%s, requestIP=%s', id, requestIP);
        ...
        if (clients[id]) {
            ...
            this.debug('Existing client details: id=%s, isOnline=%s, hasGraceTimeout=%s, originalIP=%s, age=%dms, gracePeriodRemaining=%dms',
                       id, existingClient.isOnline, !!existingClient.graceTimeout, existingClient.originalIP, clientAge, gracePeriodRemaining);
