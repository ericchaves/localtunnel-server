    ...
    async newClient(id, options = {}) {
        ...
        if (clients[id]) {
            ...
            if (existingClient.graceTimeout && !existingClient.isOnline) {
                ...
                // Only allow replacement if identifier was explicitly provided (not default)
                const hasExplicitIdentifier = options.identifier !== undefined;

                if (identifierMatch && hasExplicitIdentifier) {
                    ...
                    // Different client trying to use same subdomain OR no explicit identifier
                    // Assign random ID
                    this.debug('Client %s exists and is online, but %s mismatch (%s vs %s) or no explicit identifier, assigning random ID',
